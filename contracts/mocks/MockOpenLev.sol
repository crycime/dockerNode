...
contract MockOpenLev is OpenLevInterface {
    ...
    function marginTradeFor(
        address trader,
        OpenLevInterface.MarginTradeVars memory tradeVars,
        bytes memory dexData
    ) external payable override returns (uint256) {
        tradeVars.borrow;
        tradeVars.minBuyAmount;
        dexData;
        tradeVars.depositToken == false
            ? IERC20(_markets[tradeVars.marketId].token0).transferFrom(msg.sender, address(this), tradeVars.deposit)
            : IERC20(_markets[tradeVars.marketId].token1).transferFrom(msg.sender, address(this), tradeVars.deposit);
        Trade storage trade = _activeTrades[trader][tradeVars.marketId][tradeVars.longToken];
        trade.held += newHeld;
        trade.depositToken = tradeVars.depositToken;
        trade.deposited += tradeVars.deposit;
