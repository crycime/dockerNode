...
contract OPLimitOrder is DelegateInterface, Adminable, ReentrancyGuard, EIP712("OpenLeverage Limit Order", "1"), IOPLimitOrder,OPLimitOrderStorage {
    ...
    function fillOpenOrder(
        ...
        _assertValidEIP1271Signature(order.owner, _hashOpenOrder(order), signature);
    ...
    function fillCloseOrder(
        ...
        _assertValidEIP1271Signature(order.owner, _hashCloseOrder(order), signature);
    ...
    function _marginTrade(
        ...
        OpenLevInterface.MarginTradeVars memory tradeVars =  OpenLevInterface.MarginTradeVars(
                order.marketId,
                order.longToken,
                order.depositToken,
                fillingDeposit,
    ...
    function _assertValidEIP1271Signature(address signer,bytes32 digest,bytes memory signature) internal view {
         if (signer.code.length > 0) {
            require (EIP1271Interface(signer).isValidSignature(digest, signature) == EIP1271Interface.isValidSignature.selector ,"BCS") ;
        } else  {
            require(SignatureChecker.isValidSignatureNow(signer, digest, signature), "SNE");
        }
    }
}
